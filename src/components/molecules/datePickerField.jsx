...
const DatePickerField = ({ placeholder = 'dd/mm/yyyy' }) => {
    ...
    return (
        <>
            <TouchableOpacity style={viewStyles.InputPill} onPress={handleShowPicker}>
                <MaterialCommunityIcons name={"calendar"} size={24} color="#636363" />
                <Text style={[styles.dateFieldText, { fontSize: 16, color: !date ? "#636363" : "#000000" }]}>
                    {formatDate(date, placeholder)}
                </Text>
            </TouchableOpacity>
            <TimeDateModal isVisible={visible} onConfirm={handleConfirm} onCancel={hide} />
        </>
    );
};
